...
def prune_network(model, prune_metric, prune_ratio, pruning_iter, finetuning_epochs, ignored_layers=None):
    ...
    if ignored_layers is None:
        ...
        for name, m in model.named_modules():
            # Q2: Ignore the final linear layer
            if isinstance(m, nn.Linear): 
                ignored_layers.append(m) 
            # Q2: Ignore the first convolution layer and its batch norm
            elif name == 'conv1' or name == 'bn1':
                ignored_layers.append(m)
            # Q2: Ignore the entire first convolution block
    ...
    # TODO: Save the model
    ckpt_name = f'q2_{prune_metric}_{prune_ratio}_{pruning_iter}_{finetuning_epochs}_MBNv1.pth' # <-- Added q2_
    torch.save(model.state_dict(), ckpt_name)
    import os
    print(f"Saved checkpoint: {os.path.abspath(ckpt_name)}", flush=True)

    # TODO Export pruned model to ONNX
    onnx_name = f'q2_{prune_metric}_{prune_ratio}_{pruning_iter}_{finetuning_epochs}_MBNv1.onnx' # <-- Added q2_
    try:
        model.cpu()
