...
class LeNet5_slim(nn.Module):
    """
    Reduced-channel LeNet5 for resource-constrained devices (e.g. RPi).
    Uses 3x3 kernels with padding=1 (SAME-style) so only pooling changes
    spatial dims — making FC size predictable for any input size.

    Channels: C1: 1->4 (was 6),  C2: 4->8 (was 16)
    Spatial (16x16 input, downsampled=True):  16->pool->8->pool->4  => fc_in = 8*4*4 = 128
    Spatial (32x32 input, downsampled=False): 32->pool->16->pool->8 => fc_in = 8*8*8 = 512
    FC: fc_in -> 64 -> num_classes  (vs LeNet5's 400->120->84->10)
    """
    def __init__(self, num_classes=10, channels=1, loss_type='fedavg', downsampled=True):
        super(LeNet5_slim, self).__init__()
        self.conv1 = nn.Conv2d(channels, 4, kernel_size=3, padding=1)
        self.pool1 = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(4, 8, kernel_size=3, padding=1)
        self.pool2 = nn.MaxPool2d(2, 2)
        fc_in = 128 if downsampled else 512  # 8*4*4 or 8*8*8
