...
parser.add_argument('--dev_learning_rates', nargs='+', default=[0.01, 0.01], help='Learning rates for each device')
...
dev_learning_rates = args.dev_learning_rates
...
aux = []
for k in dev_learning_rates[0].split(" "):
    aux.append(float(k))
dev_learning_rates = aux
...
with open(path.join("configs", dev_config_filename), 'w') as file:
    ...
    for dev_idx in range(num_devices):
        ...
        dev_dict["learning_rate"] = dev_learning_rates[dev_idx]
