...
def train(model, loss_func, dev_idx, model_path, cuda_name, optimizer, local_epochs, data_iid,
          loss_type="fedavg", mu=1.0, beta=1, verbose=False, seed=42, resize=None):
    """
    ...
    Parameters:
        ...
        resize (int, optional): If set, resize images to (resize x resize) before feeding the model.

    Returns:
        tuple: The final training loss and accuracy.
    """
    device = torch.device(cuda_name)
    data_loader = load_data(data_iid=data_iid, dev_idx=dev_idx, seed=seed, resize=resize)
...
def test(model, loss_func, cuda_name, verbose=False, seed=42, loss_type='fedavg', resize=None):
    """
    ...
    data_loader = load_data(test_global=True, seed=seed, resize=resize)
...
def local_training(model_name, loss_func, loss_type, model_path, cuda_name, learning_rate, local_epochs, dev_idx,
    ...
    # Auto-detect resize: lenet5_slim expects 16x16 input
    resize = 16 if 'slim' in model_name else None
