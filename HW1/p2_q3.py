...
#added arg for running all probabilities in one run
parser.add_argument('--p', type=float, default=0.0, help='Dropout probability')
...
for epoch in range(num_epochs):
    ...
    epoch_test_acc = 100. * test_correct / test_total if test_total > 0 else 00
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'fc_metrics_{dropout_prob}.csv')
...
# After training completes, compute total time and write a summary row to table2.csv
total_training_time = time.time() - train_start_time
final_train_acc = epoch_train_acc
final_test_acc = epoch_test_acc
print(f"Total training time: {total_training_time:.2f} s | Final train acc: {final_train_acc:.2f}% | Final test acc: {final_test_acc:.2f}%")

table_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'table2.csv')
write_table_header = not os.path.exists(table_path)
try:
    with open(table_path, 'a', newline='') as tf:
        tw = csv.writer(tf)
        if write_table_header:
            tw.writerow(['dropout', 'normalize', 'train_acc_pct', 'test_acc_pct', 'total_train_time_s'])
        tw.writerow([dropout_prob, 'yes' if args.normalize else 'no', f"{final_train_acc:.4f}", f"{final_test_acc:.4f}", f"{total_training_time:.4f}"])
except Exception as e:
    print('Failed to write table1 CSV:', e)